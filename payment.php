...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  // 🔒 Tambahan Logik Semakan Tarikh
  $today = date('Y-m-d');

  // 1️⃣ Check-in mesti selepas hari ini
  if ($check_in <= $today) {
    echo "<script>alert('❌ Check-in date must be after today!'); window.history.back();</script>";
    exit;
  }

  // 2️⃣ Check-out mesti selepas check-in (minimum 1 malam)
  if ($check_out <= $check_in) {
    echo "<script>alert('❌ Check-out date must be after check-in date!'); window.history.back();</script>";
    exit;
  }

  // 3️⃣ Check booking overlap — tak boleh check-in sama dengan orang lain
  $overlap_sql = "SELECT * FROM bookings WHERE (check_in < '$check_out' AND check_out > '$check_in')";
  $overlap_result = $conn->query($overlap_sql);
  if ($overlap_result->num_rows > 0) {
    echo "<script>alert('⚠️ The selected dates are already booked by another guest. Please choose different dates.'); window.history.back();</script>";
    exit;
  }
...
<body>
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    // 🔒 JS: Halang tarikh sebelum hari ini
